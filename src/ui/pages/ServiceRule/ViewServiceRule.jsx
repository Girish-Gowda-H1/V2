...
import './ViewServiceRule.css';

const ViewServiceRule = () => {
  ...
  const handleDisable = () => {
    setLoading(true);
    axios
      .put(`${apiUrl}/service-rules/${id}/status/`, { status: 2 })
      .then((response) => {
        setLoading(false);
        setSuccess('Status Changed From Enable To Disabled.');
        setTimeout(() => {
          setSuccess('');
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <div style={{ width: '80%' }}>
          <Typography variant="h2" className="action-subtitle" sx={{ color: '#222', margin: 0, justifyContent: 'center', fontSize: '22px' }} style={{ fontSize: "22px", paddingBottom: "50px" }}>
            VIEW SERVICE RULE
          </Typography>
          <Card headerbg="#FED250" sx={{ boxShadow: '0px 0px 10px #00000034', borderRadius: '10px' }}>
            ...
            <Grid style={{ marginLeft: '8%' }}>
              <Typography variant="h3" className="action-subtitle" style={{ paddingTop: "50px" }}>
                SERVICE DETAILS
              </Typography>
          ...
          <Grid container justifyContent={'space-between'} margin={'60px 0px'}>
            ...
            <Button
              ...
              onClick={handleClose}
              style={{ color: '#5c5c5c', fontSize: '20px', letterSpacing: '1px', cursor: 'pointer', fontFamily: 'MulishBold' }}
            >
              CLOSE
            </Button>
          </Grid>
...
export default ViewServiceRule;
